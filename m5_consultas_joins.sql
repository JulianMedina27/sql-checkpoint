...
SELECT
	...
	SUM(cantidad * precio_unitario) / COUNT(id_venta) AS TicketPromedio
FROM Ventas
...
--(clientes, productos y cualquier otra dimensión de tu caso de negocio) para obtener en una sola fila, como mínimo: fecha, identificación del cliente, descripción del producto, cantidad, precio unitario y total de venta.
SELECT
    v.fecha_venta,
    v.id_venta,
    v.id_cliente,
    ...
    v.cantidad,
    v.precio_unitario,
    v.precio_unitario * v.cantidad AS TotalVenta,
    t.region,
    gen.nombre_genero
...
--Consulta 2
--Clientes sin ventas (LEFT JOIN) Identificá clientes registrados que aún no han realizado ninguna compra. Mostrá su nombre, email y fecha de registro,Usá WHERE ... IS NULL para aislar los casos.

--Agrego un cliente que no haya comprado nada para poder realizar mejor el ejercicio.
INSERT INTO clientes VALUES ('Luis Gimenez', 'Luis@mail.com', 'Buenos Aires', 'Particular', '2024-03-08', 1);
...
--Agrego un producto sin ventas para realizar mejor el ejercicio
INSERT INTO productos VALUES ('Auriculares Genius', 2,   100.00, 25, 1);
...
--Consulta 4
--Escribí dos SELECT sobre tus ventas, separados por el criterio que corresponda a tu caso (por ejemplo, ventas de dos períodos, dos sucursales o dos orígenes distintos), y agregá en cada uno una columna de texto fija que identifique el origen. 
--Unilos con UNION ALL y cerrá con un GROUP BY para obtener el total por cada origen.

SELECT
    Canal,
    SUM(ventas.TotalVenta) AS TotalFacturado
FROM
(
    SELECT
        'Fisico' AS Canal,
